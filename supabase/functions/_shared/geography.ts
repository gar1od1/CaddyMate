/**
 * Mirror of packages/api/src/geography.ts (PostGIS geography <-> LatLng):
 * PostgREST returns geography columns as hex EWKB; writes use EWKT.
 * `mirror_parity_test.ts` checks the two agree. Change the api first.
 */
import type { LatLng, Polygon } from './engine/index.ts';
