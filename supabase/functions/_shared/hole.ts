/**
 * Mirror of packages/api/src/hole.ts — the same pure code the device runs, so
 * server and device results cannot disagree. `mirror_parity_test.ts` checks
 * the two agree on shared fixtures. Change the api first, then this copy.
 */
import {
  haversineDistanceM,
  initialBearingDeg,
...
import { neutralResult, resolveElevations, type ClubProfile } from './neutral.ts';
import type { HoleScore, LieKind, Shot } from './types.ts';
