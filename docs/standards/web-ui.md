...
| `Dialog`, `ConfirmDialog`         | `components/primitives/Dialog.tsx`            | Modals; confirmations (no `window.*`)    |
...
| `/sign-in`           | Tablet-capable | Outside by design | No     | —                                                                          | —                                                                       | Yes      | Fields use `Input`                                                           |
...
| `/courses/new`       | Tablet-capable | Yes               | Yes    | —                                                                          | —                                                                       | Yes      | Fields use `Input`; area choice is radios (not a dropdown)                   |
| `/courses/[id]`      | Desktop-only   | Yes               | Editor | —                                                                          | —                                                                       | Yes      | Read-only editor; `DesktopOnlyNotice` on a phone                             |
| `/courses/[id]/edit` | Desktop-only   | Yes               | Editor | —                                                                          | Feature kind, new kind, hole: `Select`; Par, Penalty native (exception) | Yes      | Fields use `Input` (`density="sm"`); hole/tee-set delete, publish: `Dialog`s |
