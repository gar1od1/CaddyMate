/**
 * Open-Meteo request/response normalisation used by the `weather` Edge Function
 * (mirrored in supabase/functions/_shared/weather.ts so it is covered here by
 * Vitest). Lives beside the terrain code because both are the engine's view of
 * third-party environmental data; the condition model itself is in conditions/.
 */
import type { Conditions } from '../types/index.js';

/** The weather half of {@link Conditions}, plus the instant it describes. */
export type WeatherReading = Pick<
  Conditions,
  'windSpeedMps' | 'windFromDeg' | 'gustMps' | 'tempC' | 'pressureHpa'
> & {
